package com.caps.marylou.marylou;
...
import com.caps.marylou.marylou.repository.*;
import com.caps.marylou.marylou.model.Recipes;
import com.caps.marylou.marylou.model.Users;
...
public class WebUtils{
	...
	@Autowired    
	public WebUtils(JavaMailSender sender, UserRepository userRepository, RecipeRepository recipeRepository) {
		super();
		this.sender = sender;
	...
	public void addRecipePhoto(MultipartFile file, long recipeid, String folder) throws IllegalStateException, IOException {
		try {
			// save dir
			String destDir = request.getSession().getServletContext().getRealPath(UPLOADED_FOLDER)+ File.separator +folder+ File.separator +recipeid+ File.separator +"recipe";
			//innitalize file
			...
			File doc = new File(destDir + File.separator + fileName);
			//rename file
			File rename = new File(destDir + File.separator + "recipe_"+recipeid+".jpg");
			doc.renameTo(rename);
			//save to file system
			multipartFile.transferTo(rename);

			Recipes recipe=recipeRepository.findByrecipeid(recipeid);
			recipe.setImage("recipe_"+recipeid+".jpg");
